...
return function (Enrutador $enrutador): void {
    ...
    $rutas = [
        ["GET","/admin"],["GET","/admin/cache"],["POST","/admin/cache/limpiar"],
        ["GET","/admin/usuarios"],["GET","/admin/usuarios/editar"],
        ["POST","/admin/usuarios/actualizar"],["POST","/admin/usuarios/eliminar"]
    ];
    foreach ($rutas as [$m, $r]) {
        $enrutador->agregarMiddlewareRuta($m, $r, new MiddlewareRol("admin"));
    }
};
