...
class AdminControlador extends ControladorBase
{
    ...
    public function index(): void
    {
        ...
        $this->renderizarAdmin('admin/inicio',['titulo'=>'Dashboard','totalUsuarios'=>count($usuarios),'totalAdmins'=>$totalAdmins,'cacheActivo'=>true,'archivosCache'=>$est['total_archivos'],'adminBase'=>$this->urlAdminBase()]);
    }
    public function cache(): void { $cache = new Cache(); $this->renderizarAdmin('admin/cache',['titulo'=>'Caché','estadisticas'=>$cache->estadisticas(),'adminBase'=>$this->urlAdminBase()]); }
    public function limpiarCache(): void { (new Cache())->limpiar(); header('Location: '.$this->urlAdminBase().'/cache?limpiado=1'); exit; }
